...
module Scroll = {
  ...
  [@bs.send] external scrollBy: (Dom.window, opts) => unit = "scrollBy";
...
module Make = (Item: SelectableItem) => {
  ...
  let useSelection = () => {
    ...
    let (state, dispatch) =
      ReactHooks.useReducer(initialState, (state, action) =>
        [@log]
        (
          switch (action) {
          | SelectOne(id) =>
          ...
          | RestorePrevious =>
            Update({current: state.previous, previous: state.current})
...

    {
      ...
      isOnly: itemId =>
        state.current->Set.size == 1 && state.current->Set.has(itemId),
