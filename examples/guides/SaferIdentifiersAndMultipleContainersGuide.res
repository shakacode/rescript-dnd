...
let reducer = (state, action) =>
  ...
  | ReorderTodos(Some(SameContainer(todoId, placement))) =>
    let todo = state.todoMap->Belt.Map.getExn(todoId)
    {
      ...state,
      todoListMap: // Updating todos index of todo list
      state.todoListMap->Belt.Map.update(todo.todoListId, todoList =>
  ...
  | ReorderTodos(Some(NewContainer(todoId, targetTodoListId, placement))) =>
    let todo = state.todoMap->Belt.Map.getExn(todoId)
    {
      ...
      state.todoMap->Belt.Map.update(todoId, todo =>
      ...
      ->Belt.Map.update(todo.todoListId, todoList =>
        todoList->Option.map(todoList => {
          ...todoList,
          todos: todoList.todos->Array.filter(
            todoId' => todoId'->TodoId.toInt != todoId->TodoId.toInt,
          ),
        })
      )
      // Inserting todoId into the todos index of the target todo list
      ->Belt.Map.update(targetTodoListId, todoList =>
        todoList->Option.map(todoList => {
          ...todoList,
